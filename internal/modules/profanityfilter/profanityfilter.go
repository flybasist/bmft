...
func (m *ProfanityFilterModule) RegisterCommands(bot *telebot.Bot) {
	bot.Handle("/profanity", func(c telebot.Context) error {
		...
		msg += "🔹 `/setprofanity [действие]` — Включить фильтр мата (только админы)\n"
		msg += "   Действия: delete, warn, delete_warn\n"
		msg += "🔹 `/profanitystatus` — Проверить статус фильтра\n"
		msg += "🔹 `/removeprofanity` — Отключить фильтр мата (только админы)\n\n"
		msg += "🛡️ *VIP-защита:* VIP-пользователи не попадают под фильтр мата."
