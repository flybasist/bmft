...
func (m *ProfanityFilterModule) RegisterCommands(bot *telebot.Bot) {
	bot.Handle("/profanity", func(c telebot.Context) error {
		...
		msg += "🔹 <code>/setprofanity [действие]</code> — Включить фильтр мата (только админы)\n"
		msg += "   Действия: delete, warn, delete_warn\n"
		msg += "🔹 <code>/profanitystatus</code> — Проверить статус фильтра\n"
		msg += "🔹 <code>/removeprofanity</code> — Отключить фильтр мата (только админы)\n\n"
		msg += "🛡️ <i>VIP-защита:</i> VIP-пользователи не попадают под фильтр мата."
