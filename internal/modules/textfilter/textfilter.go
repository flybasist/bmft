...
func (m *TextFilterModule) RegisterCommands(bot *telebot.Bot) {
	...
	bot.Handle("/textfilter", func(c telebot.Context) error {
		...
		msg := "🚫 <b>Модуль TextFilter</b> — Фильтр запрещённых слов\n\n"
		msg += "Автоматическое удаление сообщений с запрещёнными словами и фразами.\n\n"
		msg += "<b>Доступные команды:</b>\n\n"

		msg += "🔹 <code>/addban &lt;паттерн&gt; [действие]</code> — Добавить бан-слово (только админы)\n"
		...
		msg += "   • <code>/addban (мат|ругательство) delete_warn</code> — удалять и предупреждать\n"
		msg += "   • <code>/addban реклама warn</code> — только предупреждение, без удаления\n"
		msg += "   • <code>/addban (?i)bad_word delete</code> — без учёта регистра\n\n"

		msg += "🔹 <code>/listbans</code> — Список всех запрещённых слов\n"
		msg += "   Показывает все активные фильтры с их ID и действиями\n"
		msg += "   📌 Пример: <code>/listbans</code>\n\n"

		msg += "🔹 <code>/removeban &lt;ID&gt;</code> — Удалить бан-слово (только админы)\n"
		msg += "   ID можно узнать из команды /listbans\n"
		msg += "   📌 Пример: <code>/removeban 7</code>\n\n"

		msg += "⚙️ <b>Работа с топиками:</b>\n"
		msg += "• Команда в <b>топике</b> настраивает фильтры только для этого топика\n"
		msg += "• Команда в <b>основном чате</b> настраивает фильтры для всего чата\n"
		msg += "• Если фильтр для топика не установлен, используется общий фильтр чата\n\n"

		msg += "⚠️ <b>Виды действий:</b>\n"
		...
		msg += "🛡️ <i>VIP-защита:</i> VIP-пользователи не попадают под фильтр запрещённых слов."
