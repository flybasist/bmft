...
func (m *SchedulerModule) RegisterCommands(bot *tele.Bot) {
	...
	bot.Handle("/scheduler", func(c tele.Context) error {
		...
		msg += "🔹 <code>/addtask &lt;cron&gt; &lt;текст&gt;</code> — Добавить задачу (только админы)\n"
		msg += "   📌 Пример: <code>/addtask 0 9 * * * Доброе утро!</code>\n\n"
		...
		msg += "• Команда в <b>топике</b> создаёт задачу для этого топика\n"
		msg += "• Команда в <b>основном чате</b> создаёт задачу для всего чата\n\n"

		msg += "📅 <b>Формат cron:</b> минута час день месяц день_недели\n"
		msg += "Примеры: <code>0 9 * * *</code> (каждый день в 9:00), <code>0 */6 * * *</code> (каждые 6 часов)\n\n"

		msg += "💡 <i>Подсказка:</i> Используйте сайт crontab.guru для проверки cron-выражений."
