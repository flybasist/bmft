...
**Новая функция:** Реакции для конкретных пользователей (пасхалки в стиле RTS bot)

- ✅ Добавлено поле `user_id BIGINT DEFAULT NULL` в таблицу `keyword_reactions`
- ✅ Fallback логика: `user+topic → user+chat → topic → chat` (4 уровня приоритета)
- ✅ Команда `/addreaction user:<user_id> <pattern> <response>` для персональных реакций
- ✅ Пустой паттерн (`""`) = срабатывает на **любое сообщение** пользователя
- ✅ Отображение `user_id` в `/listreactions` для персональных реакций
- ✅ Partial index для оптимизации: `idx_keyword_reactions_user`

**Пример использования:**
```bash
# Пасхалка "@Astrolux, опять ты что то спылесосил!" из RTS bot
/addreaction user:303724504 "" "@Astrolux, опять ты что то спылесосил!" "Пасхалка"
```

**Документация:**
- 📄 `docs/PERSONAL_REACTIONS.md` — подробное руководство
- 📄 `examples/personal_reaction_easter_egg.sql` — SQL примеры

**Миграция из RTS bot:**
- Старая логика: `user_id='303724504'` + `regex='disable'` в отдельной таблице
- Новая логика: `user_id=303724504` + `pattern=''` в единой таблице `keyword_reactions`

### 🔄 Изменено

- **`reactions.go::loadReactions()`** — добавлен параметр `userID int64` и сложная fallback логика
- **`reactions.go::OnMessage()`** — убрана проверка `msg.Text == ""` для поддержки реакций на фото/видео без текста
- **`reactions.go::handleAddReaction()`** — парсинг префикса `user:<user_id>` и поддержка пустого паттерна
- **`reactions.go::handleListReactions()`** — отображение `user_id` для персональных реакций
- **`migrations/001_initial_schema.sql`** — добавлено поле `user_id` + partial index

### 📚 Документация

- Обновлена справка `/reactions` с примерами персональных реакций
- Создан `PERSONAL_REACTIONS.md` с полным описанием функционала
- Создан `personal_reaction_easter_egg.sql` с SQL примерами

---

## [0.8.0-dev] - 2025-11-10

### 🗄️ КРИТИЧЕСКИЙ РЕФАКТОРИНГ БД — Единый источник правды + Поддержка топиков

**Философия:** Одна таблица `messages` с JSONB metadata вместо десятка счётчиков + нативная поддержка топиков через `thread_id`

**Проблема (v0.7):**
- 13 таблиц (messages + content_counters + reaction_triggers + reaction_daily_counters и т.д.)
- Дублирование данных (счётчики живут отдельно от сообщений)
- Сложная синхронизация (обновить счётчик + записать сообщение)
- Каждый новый модуль добавляет 2-3 таблицы
- **Отсутствие поддержки топиков (Telegram Topics/Forums)**

**Решение (v0.8):**
```
13 таблиц → 9 таблиц + 2 MATERIALIZED VIEW
+ thread_id в 6 таблицах для поддержки топиков
...
2. `thread_id > 0` — настройка только для конкретного топика
3. При отсутствии настройки для топика → fallback на настройки чата
...
**Создано:**
- ✅ `MessageRepository` (235 строк) — единый репозиторий для работы с messages
  - `InsertMessage(chatID, threadID, ...)` — сохранение с thread_id + metadata
  - `GetTodayCountByType(chatID, threadID, ...)` — счётчики для Limiter
  - `GetUserStats(chatID, threadID, ...)` — статистика для Statistics
  - `GetChatTopUsers(chatID, threadID, ...)` — топ активных
...
- ✅ `Limiter module` — использует `messageRepo.GetTodayCountByType(chatID, threadID, ...)`
  - Лимиты задаются командой `/setlimit` в топике → действуют только в топике
  - VIP через `/setvip` в топике → VIP только в топике
  - Fallback: нет настроек в топике → применяются настройки чата

- ✅ `migrations.go` — упрощена логика (только 001_initial_schema.sql)
  - ExpectedSchema обновлён (thread_id в 6 таблицах)

#### ✅ Telegram API Integration:

- ✅ Используется `Message.ThreadID` из gopkg.in/telebot.v3
- ✅ `threadID = 0` → основной чат
- ✅ `threadID > 0` → сообщение в топике
  - Удалена функция `checkAndRunMigration002` (больше нет миграции 002)
  - Обновлён `ExpectedSchema` — убраны счётчики

**Обоснование:**
- **Гибкость:** новый модуль добавляет ключ в metadata без миграций
- **Производительность:** GIN индексы + MATERIALIZED VIEW быстрее чем JOIN
- **Простота:** меньше таблиц = меньше сложности
- **Истина:** одно место хранения данных о сообщениях

**Результат:** 
- Схема БД упрощена с 13 до 9 таблиц
- Единый источник правды — `messages` с JSONB metadata
- MATERIALIZED VIEW для быстрой аналитики
- Готовность к масштабированию (партиционирование + GIN)
