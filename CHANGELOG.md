...
### 🧹 Рефакторинг кода - 2025-11-02

**Цель:** Очистка кодовой базы от неиспользуемых функций и актуализация документации

**Выполнено:**
- ✅ Удалены неиспользуемые функции из `settings.go`: `SetVersion()`, `SetTimezone()`, `GetTimezone()` (оставлена только `GetVersion()`)
- ✅ Удалена неиспользуемая функция `IsActive()` из `chat_repository.go`
- ✅ Удалены системные файлы `.DS_Store` из репозитория
- ✅ Обновлён `README.md`:
  - Добавлен раздел **"📦 Доступные модули"** с таблицей всех модулей
  - Добавлен раздел **"⚙️ Конфигурация"** с примером `.env`
  - Добавлен раздел **"🔧 Основные команды"** для пользователей и администраторов
- ✅ Проверено соответствие правилам проекта:
  - Все комментарии в коде на русском ✓
  - Все логи в консоль на английском ✓
  - Сообщения пользователю на русском ✓

**Результат:** Удалено ~50 строк мёртвого кода, улучшена читаемость документации
