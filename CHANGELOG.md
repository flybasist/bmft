...
## v1.3 — Лимиты и запреты для Inline-ботов (2026-06-14)

### Поддержка via_bot

- **Новый тип контента:** В `DetectContentType` добавлен тип `via_bot` для всех сообщений, отправленных через инлайн-ботов (с плашкой `via @botname`).
- **Свой лимит:** Теперь можно лимитировать количество сообщений от инлайн-ботов (команда `/setlimit via <число>` или через меню в боте).
- **Интерфейс:** В экранах статистики и просмотра лимитов появилась новая строка 🤖 **Inline боты**.
- **Блокировка ботов:** Добавлена новая команда `/addviaban @botname <комментарий>`, которая позволяет полностью запретить использование конкретного инлайн-бота в чате или топике. Бот-модератор удалит сообщение и ответит вашим комментарием.

### Миграция 006

- Добавлена колонка `limit_via` в таблицу `content_limits`.
- Обновлена версия `bot_settings.bot_version` на `1.3`.
