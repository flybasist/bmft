...
### Added
- ✅ **Statistics Module** — сбор и отображение статистики активности пользователей
  - `internal/postgresql/repositories/statistics_repository.go` — StatisticsRepository (310 строк, 5 методов)
  - `internal/modules/statistics/statistics.go` — StatisticsModule (520 строк)

### Features
- 📊 **Автоматический сбор статистики:** Инкремент счётчиков при каждом сообщении (UPSERT в statistics_daily)
- 📊 **Типы контента:** text, photo, video, sticker, voice, document, audio, animation, video_note, location, contact, poll, other
- 📊 **Личная статистика:** /mystats (за сегодня), /myweek (за последние 7 дней)
- 📊 **Статистика чата:** /chatstats (за сегодня, только админы)
- 📊 **Топ пользователей:** /topchat (топ-10 активных за день, только админы)
- 📊 **Форматирование:** HTML-форматированный вывод с эмодзи

### Commands
- `/mystats` — Посмотреть свою статистику за сегодня
- `/myweek` — Посмотреть свою статистику за неделю
- `/chatstats` — (Админ) Статистика чата за сегодня
- `/topchat` — (Админ) Топ-10 активных пользователей

### Database
- Использует существующую таблицу `statistics_daily` из миграции 001
- UPSERT операции для атомарного инкремента счётчиков
- Агрегация по типам контента через CASE WHEN

### Technical Details
- **Repository методы:**
  - `IncrementCounter()` — увеличить счётчик сообщений (UPSERT)
  - `GetUserStats()` — статистика пользователя за день
  - `GetUserWeeklyStats()` — статистика за последние 7 дней
  - `GetChatStats()` — статистика всего чата за день
  - `GetTopUsers()` — топ активных пользователей (с ranking)
- **Модуль интегрирован:** Зарегистрирован в Module Registry, команды добавлены в бота
- **Обработка сообщений:** OnMessage() детектит тип контента и инкрементит счётчик

### Documentation
- 📝 README.md обновлён: Statistics модуль отмечен как завершённый ✅
