...
- 📝 **README.md:** Удалены устаревшие упоминания о будущих модулях
- 📝 **cmd/bot/main.go:** Обновлена команда /help с актуальным списком модулей
- 📝 **Все комментарии в коде:** Проверены и приведены в соответствие с реализацией

## [0.6.0] - 2025-10-05 (Phase 2.5 + Phase 3.5 + Welcome Module)

### Added
- ✅ **Welcome Module** — приветственные сообщения и информация о боте
  - `internal/core/welcome.go` — модуль приветствий (100 строк)
  - **Python паритет:** `reaction.py::newchat()`, `reaction.py::newmember()`, `reaction.py::reactionversion()`
  - **Команды:** `/version` — информация о версии бота и контакты разработчика
  - **Features:** Приветствие новых пользователей, сообщение при добавлении бота в чат
  - **Integration:** Обработчик события OnUserJoined, регистрация команд в main.go
...
- ✅ **Edited Message Handler** — обработка отредактированных сообщений
  - Обработчик `OnEdited` в `cmd/bot/main.go` — передаёт edited messages всем модулям
  - **Python паритет:** `@bot.edited_message_handler()` — обрабатывает точно так же как новое сообщение
  - **Integration:** Отредактированные сообщения проходят через registry.OnMessage()
...
### Features
- 🎉 **100% Python Parity Achieved** — все 28 функций из rts_bot мигрированы
...
- Phase 2.5: Content Type Limiter (photo/video/sticker/etc)
- Phase 3: Reactions Module (regex/exact/contains)
- Phase 3.5: Text Violations Counter (violation_code=21)
- Phase 4: Statistics Module (mystats/chatstats/topchat)
