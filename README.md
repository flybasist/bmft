...
## 🔴 **Критические Phase (паритет с Python rts_bot)**

Перед переключением на Go версию **ОБЯЗАТЕЛЬНО** нужно реализовать:

### Phase 2.5 (✅ Завершена) — Content Type Limiter
**Статус:** ✅ Реализовано  
...
### Phase 3.5 (🔴 КРИТИЧНО!) — Text Violations Counter
**Статус:** 🟡 Частично в Reactions Module  
**Python аналог:** `checkmessage.py::regextext()` — счётчик текстовых нарушений

- [ ] Расширить `reactions_log` для подсчёта violations
- [ ] Счётчик нарушений (violation_code=21) за день
- [ ] Удаление сообщений при превышении `limitviolation`
- [ ] Предупреждения при приближении к лимиту
- [ ] Интеграция с Reactions Module (использовать violation_code)
- [ ] Команда: `/mytextviolations` — показать счётчик за день

**⚠️ БЕЗ ЭТОГО Phase пропадёт модерация текстовых нарушений!**
