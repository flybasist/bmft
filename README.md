# BMFT - Bot Moderator For Telegram 📖 О проекте

BMFT — модульный бот для управления Telegram-чатами с plugin-based архитектурой.

- 🧩 **Модульная архитектура** — независимые модули (лимитер, реакции, статистика и др.)
- 🎛️ **Per-chat управление** — админы включают/выключают модули командами
- 🗄️ **PostgreSQL** — единая БД с партиционированием
- 📡 **Long Polling** — без webhook и публичного IP
- 🐳 **Docker-ready** — простое развертывание

> 🟡 **Статус:** Alpha. Активная разработка.

## 🏗️ Архитектура (v0.7-simplified)

**Упрощённый монолит** — явный pipeline обработки сообщений без магических абстракций

```
┌─────────────────────────────┐
│        Telegram API         │
└─────────────┬───────────────┘
              │ Long Polling
              ▼
        ┌─────────────────────┐
        │   Bot (telebot.v3)  │
        └─────────┬───────────┘
                  │
             ┌────┴────┐
             │ Pipeline│ ← Явный порядок: limiter→textfilter→statistics→reactions
             │ (cmd/   │   Конфигурация per-chat через chat_modules (БД)
             │  bot/)  │
             └────┬────┘
                  │
     ┌────────────┼────────────┬────────────┐
     ▼            ▼            ▼            ▼
┌─────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐
│ Limiter │ │TextFilter│ │Statistics│ │Reactions │
│ (VIP    │ │ (banned  │ │ (content │ │ (keyword │
│  bypass)│ │  words)  │ │ counters)│ │  reply)  │
└────┬────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘
     └───────────┴────────────┴────────────┘
                  │
        ┌─────────┴─────────┐
        │    Scheduler      │ ← Отдельный lifecycle (cron tasks)
        │  (New→Start→      │   Не участвует в pipeline сообщений
        │   Shutdown)       │
        └─────────┬─────────┘
                  │
                  ▼
        ┌─────────────────────┐
        │    PostgreSQL 16+   │
        └─────────────────────┘
```

**Ключевые изменения (v0.7):**
- ❌ Убран `Module Registry` (заменён на явный pipeline в `cmd/bot/pipeline.go`)
- ✅ Явный порядок модулей (критичен!) в `processMessage()`
- ✅ Централизованная инициализация в `cmd/bot/modules.go`
- ✅ Scheduler: явный lifecycle `New() → Start() → Shutdown()`
- ✅ Понятность > Enterprise абстракции
## � Доступные модули

| Модуль | Описание | Основные команды |
|--------|----------|------------------|
| **Limiter** | Лимиты на контент с VIP-обходом | `/setlimit`, `/mylimits`, `/setvip` |
| **Reactions** | Автореакции на ключевые слова | `/addreaction`, `/listreactions` |
| **Statistics** | Статистика активности пользователей | `/mystats`, `/myweek`, `/topweek` |
| **Scheduler** | Задачи по расписанию (cron) | `/addtask`, `/listtasks` |
| **TextFilter** | Фильтр запрещённых слов | `/addban`, `/listbans` |

Все модули включаются/выключаются админами командами `/enable <module>` и `/disable <module>`.

---

## �🚀 Быстрый старт

### Требования
- Docker и Docker Compose
- Telegram Bot Token от [@BotFather](https://t.me/BotFather)

### Установка
```bash
# 1. Клонируйте и настройте
git clone <repository-url>
cd bmft
cp .env.example .env  # Укажите TELEGRAM_BOT_TOKEN

# 2. Запустите
docker-compose -f docker-compose.env.yaml up -d  # PostgreSQL
docker-compose -f docker-compose.bot.yaml up -d  # Бот

# 3. Проверьте
docker logs -f bmft_bot
```

---

## ⚙️ Конфигурация

Создайте файл `.env` на основе `.env.example`:

```bash
# Telegram Bot Token от @BotFather
TELEGRAM_BOT_TOKEN=your_token_here

# Подключение к PostgreSQL
POSTGRES_DSN=postgres://bmft:password@postgres:5432/bmft?sslmode=disable

# Уровень логирования: debug, info, warn, error
LOG_LEVEL=info

# Pretty print логов (true для dev, false для prod)
LOG_PRETTY=true

# Таймаут polling (секунды)
POLLING_TIMEOUT=60

# Таймаут graceful shutdown (секунды)
SHUTDOWN_TIMEOUT=30

# Часовой пояс
TZ=Europe/Moscow
```

---

## 🔧 Основные команды

### Для всех пользователей:
- `/start` — запуск бота
- `/help` — список команд
- `/modules` — доступные модули
- `/mystats` — ваша статистика
- `/myweek`, `/mymonth` — статистика за период
- `/mylimits` — ваши лимиты

### Для администраторов:
- `/enable <module>` — включить модуль
- `/disable <module>` — выключить модуль
- `/setlimit <type> <limit>` — установить лимит на контент
- `/setvip @user` — дать VIP статус (обход всех лимитов)
- `/addreaction <слово> <ответ>` — добавить автореакцию
- `/addban <слово>` — забанить слово
- `/addtask <cron> <текст>` — создать задачу по расписанию

**Подсказка:** Используйте reply на сообщение с `/addreaction` или `/addtask` для автоматического определения типа контента (стикер, фото, и т.д.).

---

## 📧 Контакты

- GitHub: [@flybasist](https://github.com/flybasist)
- Telegram: [@flybasist](https://t.me/flybasist)
- Email: flybasist92@gmail.com

## 📄 Лицензия

Этот проект распространяется под лицензией GNU GPLv3.

Вы можете использовать, модифицировать и распространять этот код, при условии, что производные работы также будут открыты под лицензией GPLv3. Это означает, что если вы вносите изменения и распространяете модифицированную версию, вы обязаны предоставить исходный код этих изменений.

В случае использования кода внутри организации без его распространения — раскрытие изменений не требуется.

Автор: Alexander Ognev (aka FlyBasist)
Год: 2025

⭐ Если проект оказался полезен — поставьте звезду на GitHub! ⭐

🇺🇸 English

This project is licensed under the GNU GPLv3.

You are free to use, modify, and distribute this code under the condition that any derivative works are also licensed under GPLv3. This means if you make changes and distribute your modified version, you must make the source code of those changes available.

If you use the code within your organization without distributing it externally, you are not required to disclose your modifications.

Author: Alexander Ognev (aka FlyBasist), 2025